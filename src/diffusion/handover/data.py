...
class HandoverDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        # read from all npz files
        for npz_file in file_paths:
            data = np.load(npz_file)

            actions = data['action']
            if actions.shape[0] < 6:
                continue
                # print(actions.shape)
                # pdb.set_trace()
            euler_rot = actions
            ...
            cumulative_len += actions.shape[0]

            self.episode_ends.append(cumulative_len)
            ...
            # inpainted_im = file_data['inpainted']

        train_image_data = self.obs[:] / 255.0
