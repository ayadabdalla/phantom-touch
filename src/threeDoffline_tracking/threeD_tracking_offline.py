...
# 1. Depth images are initially in camera frame
# 2. Point clouds are transformed from camera frame -> robot base frame
# 3. CAD model initial pose is specified in robot base frame
# 4. ICP alignment is performed in robot base frame
# 5. All output positions and orientations are in robot base frame
...
cfg = OmegaConf.load(f"{current_dir}/config/threeD_tracking_offline.yaml")
mask_output_dir = cfg.mask_output_dir
sam2video_cfg= cfg.sam2videoPredictor
data_dir = cfg.data_dir
dataset_name = cfg.dataset_name
dataset_format = cfg.dataset_format
depth_data_dir = cfg.depth_data_dir
depth_shape = tuple(cfg.depth_shape)
threeD_tracking_output_dir = cfg.threeD_tracking_output_dir
...
i=1
# load images from the dataset and overlay them, #TODO: key names are hardcoded for now
...
images_path = os.path.join(data_dir, temp_dir)
if cfg.mask_objects:
    # # Save images to the temporary directory
    if not os.path.exists(images_path) or len(os.listdir(images_path)) != len(images):
    ...
    for i, frame_path in enumerate(frame_names):
        frame_name = os.path.join(mask_output_dir, f"mask_{os.path.basename(frame_path)}")
        print(f"Saving {frame_name}")
        cv2.imwrite(frame_name, mask_frames[i])

# retrieve depth data and save masked point clouds
depth_data = load_raw_depth_images(depth_data_dir, shape=depth_shape)
depth_data = depth_data[data["indexes"].astype(int)]
print(f"Loaded {len(depth_data)} depth images from {depth_data_dir}")

# Optional: Load pre-existing masks instead of generating with SAM2
# Set LOAD_EXISTING_MASKS to True to load from output_dir
LOAD_EXISTING_MASKS = True

if LOAD_EXISTING_MASKS:
    print(f"Loading existing masks from {mask_output_dir}...")
    mask_paths = sorted(
        [os.path.join(mask_output_dir, f) for f in os.listdir(mask_output_dir) if f.startswith("mask_") and f.endswith(".png")],
        key=lambda p: os.path.splitext(os.path.basename(p))[0]
    )
    mask_frames = []
...
CAMERA_TO_ROBOT_TRANSFORM_PATH = cfg.CAMERA_TO_ROBOT_TRANSFORM_PATH
...
# Convert depth images and masks to point clouds

# ============ CAD Model Configuration ============
# Path to the CAD model (mesh file: .obj, .stl, .ply, etc.)
CAD_MODEL_PATH = cfg.CAD_MODEL_PATH  # UPDATE THIS PATH
...
point_clouds = []
centroids = []
print("Converting depth images and masks to point clouds...")
print(f"Depth shape: {depth_data[0].shape}, Mask shape: {mask_frames[0].shape}")

# Get depth resolution
depth_height, depth_width = depth_data[0].shape
...
for i in range(min(len(depth_data), len(mask_frames))):
    ...
    #     depth_normalized = cv2.normalize(depth_data[i], None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
    ...
    pcd, centroid = depth_to_point_cloud(depth_data[i], mask_resized, fx, fy, cx, cy)
...
if len(point_clouds) > 0:
    ...
    FITNESS_THRESHOLD = 0.5  # Adjust this value based on your data quality needs
    ...
    for i, (pcd, centroid) in enumerate(zip(point_clouds, centroids)):
        # Center the current observation point cloud
